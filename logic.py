...
class DB_Manager:
    ...
    def update_project_photo(self, project_name, user_id, photo):
        conn = sqlite3.connect(self.database)
        with conn:
            conn.execute(
                "UPDATE projects SET photo = ? WHERE project_name = ? AND user_id = ?",
                (photo, project_name, user_id)
            )
            conn.commit()
    ...
    def insert_project(self, data):
        '''
        ЗАПИШИ ДАННЫЕ В ФОРМАТЕ: [('user_id', 'project_name', 'description', 'url', 'status_id','photo')]
        '''
